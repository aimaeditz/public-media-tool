...
async function run() {
  const chunksDir = path.resolve('./app-source/src/lib/data/category-chunks');
  ...
  console.log(`Loaded ${totalLoaded} tools from ${files.length} chunk files.`);

  // Define the target 53 categories with metadata and tool allocation logic
  ...
  console.log(`\nRebalance Plan Result:`);
  console.log(`Total Categories: ${targetCategories.length}`);
  let sumTools = 0;
  const toolSlugSet = new Set<string>();
  let dupes = 0;

  for (const cat of targetCategories) {
    console.log(`- ${cat.name.padEnd(35)} (${cat.slug.padEnd(25)}): ${cat.tools.length} tools [Dept: ${cat.department}]`);
    sumTools += cat.tools.length;
    for (const t of cat.tools) {
      if (toolSlugSet.has(t.slug)) dupes++;
      toolSlugSet.add(t.slug);
    }
  }

  console.log(`\nVerification:`);
  console.log(`Sum of tools in categories: ${sumTools} (Expected: 1516)`);
  console.log(`Unique tool slugs: ${toolSlugSet.size} (Duplicates: ${dupes})`);
  console.log(`Min tools in category: ${Math.min(...targetCategories.map(c => c.tools.length))}`);
  console.log(`Max tools in category: ${Math.max(...targetCategories.map(c => c.tools.length))}`);
}
