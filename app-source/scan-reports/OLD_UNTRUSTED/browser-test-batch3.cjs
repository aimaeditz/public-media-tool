...
const logFilePath = path.join(__dirname, '../scan-reports/runlogs/tools-151-200-browser.log');
const testWavPath = path.join(__dirname, '../scan-reports/runlogs/test_sample.wav');

fs.mkdirSync(path.dirname(logFilePath), { recursive: true });

fs.writeFileSync(logFilePath, `=== STARTING HEADLESS CHROMIUM BROWSER AUDIT FOR TOOLS 151 TO 200 ===\nTimestamp: ${new Date().toISOString()}\nBase URL: http://localhost:3000/#/tools/\n\n`, 'utf8');
...
(async () => {
  ...
  await browser.close();
  appendLog(`\n=== COMPLETED BROWSER TEST FOR TOOLS 151 TO 200 ===`);
})();
