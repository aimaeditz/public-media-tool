...
const logFilePath = path.join(__dirname, '../scan-reports/runlogs/tools-201-300-browser.log');
const testWavPath = path.join(__dirname, '../scan-reports/runlogs/test_sample.wav');

fs.mkdirSync(path.dirname(logFilePath), { recursive: true });

fs.writeFileSync(logFilePath, `=== STARTING HEADLESS CHROMIUM BROWSER AUDIT FOR TOOLS 201 TO 300 ===\nTimestamp: ${new Date().toISOString()}\nBase URL: http://localhost:3000/#/tools/\n\n`, 'utf8');
...
(async () => {
  ...
  await browser.close();
  appendLog(`\n=== COMPLETED BROWSER TEST FOR TOOLS 201 TO 300 ===`);
})();
