...
const audioToolsUi = `
            ...
            <div className="bg-white p-8 rounded-2xl border border-slate-200 text-center space-y-6">
              ...
              <div className="flex justify-center">
                <button
                  ...
                  className="w-44 h-44 rounded-full bg-linear-to-tr from-purple-700 to-indigo-600 hover:from-purple-800 hover:to-indigo-700 text-white font-extrabold text-xl shadow-lg active:scale-95 transition-transform flex flex-col items-center justify-center gap-1 cursor-pointer"
                >
      ...
      {slug === 'reverb-time-calculator-decay' && (() => {
        ...
        const preDelayMs = Math.round((quarterMs / 8) * 10) / 10; // 1/32 note pre-delay
        const baseRt60Map: Record<string, number> = {
          booth: 0.45,
          room: 0.95,
          hall: 2.1,
          cathedral: 4.5,
        ...
        const rt60Sec = Math.round((baseRt60Map[revRoomType] || 2.0) * revDecayMultiplier * 100) / 100;
      ...
      {slug === 'chord-progression-generator-smart' && (() => {
        ...
        const currentProg = genrePresets[chordGenre] || genrePresets.pop;
              ...
              {currentProg.chords.map((c, i) => (
                <div key={i} className="bg-white p-5 rounded-2xl border border-slate-200 text-center space-y-3 hover:border-purple-300 transition-colors">
                  ...
                  <button
                    ...
                    className="w-full py-1.5 bg-purple-50 hover:bg-purple-100 text-purple-700 rounded-lg text-xs font-bold flex items-center justify-center gap-1.5 transition-colors"
                  >
      ...
      {slug === 'audio-pan-law-calculator' && (() => {
        const normPan = panPosition / 100; // -1 to +1
        const theta = ((normPan + 1) / 2) * (Math.PI / 2); // 0 to pi/2
...
// Insert right before the last </div> of return
const targetStr = '{/* PROCESSED AUDIO EXPORT BANNER */}';
if (code.includes(targetStr) && !code.includes('BPM TEMPO FINDER PRECISION')) {
  code = code.replace(targetStr, audioToolsUi + '\n      ' + targetStr);
  fs.writeFileSync('app-source/src/components/tools/AudioMusicToolsRunner.tsx', code, 'utf8');
  console.log('Successfully inserted UI blocks for tools 13-20');
} else {
  console.log('Target string not found or already inserted');
}
