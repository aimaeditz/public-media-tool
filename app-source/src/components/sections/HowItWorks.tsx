...
export const HowItWorks: React.FC = () => {
  ...
  return (
    <section className="py-10 sm:py-20 md:py-28 bg-gradient-to-b from-slate-950 via-slate-900 to-indigo-950 text-white border-b border-slate-800 relative overflow-hidden">
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-2xl mx-auto mb-8 sm:mb-16 space-y-2 sm:space-y-3">
          <span className="inline-flex items-center gap-1.5 text-[11px] sm:text-xs font-bold uppercase tracking-widest text-indigo-300 px-3.5 py-1.5 bg-indigo-500/10 rounded-full border border-indigo-500/20 shadow-xs">
        ...
        <div className="relative grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-8 lg:gap-10">
          ...
          {steps.map((step, idx) => {
            ...
            return (
              <div
                key={idx}
                className="group relative z-10 bg-slate-900/90 backdrop-blur-md rounded-2xl p-5 sm:p-8 border border-slate-800 hover:border-indigo-500/40 transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-indigo-500/10 flex flex-col justify-between overflow-hidden"
              >
                {/* Large Background Watermark Step Number */}
                <span className="absolute top-2 right-4 text-5xl sm:text-7xl font-black font-heading text-slate-800/40 select-none pointer-events-none group-hover:text-indigo-500/20 transition-colors">
                  {step.num}
                </span>
                ...
                <div className="space-y-4 sm:space-y-6 relative z-10">
                  ...
                  <div className="flex items-center justify-between">
                    <div className={`w-11 h-11 sm:w-14 sm:h-14 rounded-2xl bg-gradient-to-br ${step.gradient} text-white flex items-center justify-center shadow-lg ${step.badgeGlow} group-hover:scale-110 transition-transform`}>
                      <IconComp className="w-5 h-5 sm:w-7 sm:h-7" />
                    </div>

                    <span className="text-[10px] font-bold px-2.5 sm:px-3 py-0.5 sm:py-1 bg-slate-800 text-slate-300 rounded-full border border-slate-700/80">
                      {step.tag}
                    </span>
                  </div>
                ...
                <div className="pt-4 sm:pt-6 mt-4 sm:mt-6 border-t border-slate-800/80 flex items-center justify-between text-xs text-indigo-300 font-semibold relative z-10">
                  <span className="inline-flex items-center gap-1.5 text-[11px] sm:text-xs">
                    <ShieldCheck className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-400" />
                    {step.highlight}
                  </span>
                  <ArrowRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-500 group-hover:text-indigo-400 group-hover:translate-x-1 transition-all" />
                </div>
              </div>
            );
          })}
