...
export const ToolRunner: React.FC<ToolRunnerProps> = ({ tool }) => {
  ...
  switch (tool.slug) {
    ...
    default:
      return <ComingSoonTool tool={tool} onCopy={triggerCopyNotice} copied={copied} />;
  }
};
...
/* =========================================================================
   23. COMING SOON TOOL FALLBACK
   ========================================================================= */
const ComingSoonTool: React.FC<{ tool: Tool; onCopy: () => void; copied: boolean }> = ({ tool, onCopy, copied }) => {
  const [requested, setRequested] = useState(false);
  const [text, setText] = useState('');

  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-br from-indigo-50 to-purple-50 border border-indigo-100 p-8 rounded-2xl text-center space-y-4">
        <div className="w-12 h-12 mx-auto rounded-2xl bg-indigo-600 text-white flex items-center justify-center shadow-md">
          <Sparkles className="w-6 h-6" />
        </div>
        <div className="space-y-1">
          <h3 className="text-xl font-bold font-heading text-slate-900">{tool.name} is Coming Soon!</h3>
          <p className="text-xs text-slate-600 max-w-md mx-auto">
            We are fine-tuning this tool for 100% client-side privacy and lightning speed. In the meantime, you can use our universal text scratchpad below.
          </p>
        </div>
        ...
        <button
          ...
          disabled={requested}
          className="px-5 py-2.5 text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-700 disabled:bg-emerald-600 rounded-xl transition-all shadow-sm inline-flex items-center gap-2 cursor-pointer"
        >
          {requested ? <Check className="w-4 h-4" /> : <Sparkles className="w-4 h-4" />}
          {requested ? 'Priority Access Requested!' : 'Notify Me When Ready'}
        </button>
      </div>

      <div className="bg-white p-6 rounded-2xl border border-slate-200 space-y-3">
        <div className="flex items-center justify-between">
          <h4 className="font-bold text-slate-800 text-sm">Universal Text Scratchpad</h4>
          <button
            onClick={() => {
              navigator.clipboard.writeText(text);
              onCopy();
            ...
            disabled={!text}
            className="px-3 py-1.5 text-xs font-medium text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 rounded-lg flex items-center gap-1"
          >
            {copied ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
            {copied ? 'Copied!' : 'Copy Scratchpad'}
          </button>
        </div>
        <textarea
          value={text}
          onChange={(e) => setText(e.target.value)}
          rows={6}
          placeholder="Use this quick scratchpad while this tool is being updated..."
          className="w-full p-3 text-sm rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/30"
        />
      </div>
    </div>
  );
